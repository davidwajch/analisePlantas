...
def processar_imagem(image, img: bool):
    ...
    dead_percentage = round((affected_area / total_area) * 100, 2)
    dead_tot += dead_percentage

    if count % 20 == 1:
        avg_dead_percentage = dead_tot / count

        if avg_dead_percentage < 10:
            print("A porcentagem de folha morta é baixa:", avg_dead_percentage, "%")
            print("Nenhuma ação imediata é necessária.")
        elif avg_dead_percentage < 50:
            print("A porcentagem de folha morta é moderada:", avg_dead_percentage, "%")
            print("Recomenda-se realizar ações de controle e tratamento adequados.")
        else:
            print("A porcentagem de folha morta é alta:", avg_dead_percentage, "%")
            print("Ações de controle e tratamento devem ser tomadas urgentemente.")
    elif img:
        if dead_percentage < 10:
            print("A porcentagem de folha morta é baixa:", dead_percentage, "%")
            print("Nenhuma ação imediata é necessária.")
        elif dead_percentage < 50:
            print("A porcentagem de folha morta é moderada:", dead_percentage, "%")
            print("Recomenda-se realizar ações de controle e tratamento adequados.")
        else:
            print("A porcentagem de folha morta é alta:", dead_percentage, "%")
            print("Ações de controle e tratamento devem ser tomadas urgentemente.")
